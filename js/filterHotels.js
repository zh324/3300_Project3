    ...
    if (hotel == true) {
        ...
        if (hotel_filtered_data.length == 0) {
            ...
            for (var i = 0; i < hotel_filtered_data.length; i++) {
                var object = addSplineObject("hotel", new THREE.Vector3(hotel_filtered_data[i].coordinate[0], -150, hotel_filtered_data[i].coordinate[1]), hotel_filtered_data[i]["Company Name"], hotel_filtered_data[i]["Price"], hotel_filtered_data[i]["Rating"], hotel_filtered_data[i]["Region"], hotel_filtered_data[i]["Address"], "green");
                objectCount++;
